...
def add_base_max_kernel(
    ...
    if pid > 0:
        ...
        # NaN-aware maximum (same semantics as maximum_with_index_tie_break_right)
        use_cur = out_vals > last_part_max_via_max
        equal = out_vals == last_part_max_via_max
        cur_is_nan = out_vals != out_vals
        prev_is_nan = last_part_max_via_max != last_part_max_via_max
        use_cur |= cur_is_nan & ~prev_is_nan
        equal |= cur_is_nan & prev_is_nan
        use_cur |= equal & (out_indices > last_part_max_index_via_max)
...
def scan_part_max_kernel(
    ...
    if tl.constexpr(NEED_PARTIAL):
        ...
        if tl.constexpr(not USE_OUT_INDICES):
            part_max_indices_via_max = pid * BLOCK_SIZE + part_max_indices_via_max
...
def scan_part_max_abc_kernel(
    ...
    if tl.constexpr(NEED_PARTIAL):
        ...
        if tl.constexpr(not USE_OUT_INDICES):
            part_max_indices_via_max = pid_b * BLOCK_SIZE + part_max_indices_via_max
...
def cummax(
    ...
    *,
    ...
    logger.debug("GEMS_ASCEND CUMMAX")
