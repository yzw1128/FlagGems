...
@triton.heuristics(_hcu.HEURISTICS_CONFIGS["log_normal"])
...
def log_normal(x, mean=1.0, std=2.0, *, generator=None):
    logger.debug("GEMS_ASCEND LOG_NORMAL")
    dtype = x.dtype
    device = x.device
    ...
    with torch_device_fn.device(device):
        if not _fast_launch(res, N, mean, std, philox_seed, philox_offset, UNROLL):
            log_normal_kernel[grid_fn](
                res, N, mean, std, philox_seed, philox_offset, UNROLL
            )
    return res
...
def _fast_launch(res, N, mean, std, philox_seed, philox_offset, UNROLL):
    if res.data_ptr() % 16 != 0:
        return False
    jit_fn = log_normal_kernel.fn
    if not hasattr(jit_fn, "warmup"):
        return False
    BLOCK = _hcu.log_normal_heur_block({"N": N})
    num_warps = _hcu.log_normal_heur_num_warps({"N": N})
    ...
    try:
        ...
        compiled[(grid0, 1, 1)](res, N, mean, std, philox_seed, philox_offset, UNROLL)
    except Exception:
