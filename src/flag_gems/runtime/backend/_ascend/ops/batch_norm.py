...
def _block_m(batch_dim):
    return min(64, triton.next_power_of_2(batch_dim))


def _block_n(batch_dim, spatial_dim):
    BLOCK_M = _block_m(batch_dim)
    BLOCK_N = triton.next_power_of_2(spatial_dim)
...
def batch_norm(
    ...
    BM = _block_m(batch_dim)
    BN = _block_n(batch_dim, spatial_dim)
