...
def add_base_min_kernel(
    ...
    if pid > 0:
        ...
        use_cur = out_vals < last_part_min_via_min
        equal = out_vals == last_part_min_via_min
        cur_is_nan = out_vals != out_vals
        prev_is_nan = last_part_min_via_min != last_part_min_via_min
        use_cur |= cur_is_nan & ~prev_is_nan
        equal |= cur_is_nan & prev_is_nan
        use_cur |= equal & (out_indices > last_part_min_index_via_min)

        final_vals = tl.where(use_cur, out_vals, last_part_min_via_min)
        final_indices = tl.where(use_cur, out_indices, last_part_min_index_via_min)
...
def scan_part_min_kernel(
    ...
    if tl.constexpr(NEED_PARTIAL):
        ...
        if tl.constexpr(not USE_OUT_INDICES):
            part_min_indices_via_min = pid * BLOCK_SIZE + part_min_indices_via_min
...
def scan_part_min_abc_kernel(
    ...
    if tl.constexpr(NEED_PARTIAL):
        ...
        if tl.constexpr(not USE_OUT_INDICES):
            part_min_indices_via_min = pid_b * BLOCK_SIZE + part_min_indices_via_min
...
def cummin(
    ...
    *,
    ...
    logger.debug("GEMS_ASCEND CUMMIN")
