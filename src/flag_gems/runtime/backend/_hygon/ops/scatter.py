...
def _generate_generic_offsets(code: IndentedBuffer, rank: int) -> None:
...
def generate_scatter_kernel(
    ...
    with code.indent():
        code.writeline("if flag_gems.vendor_name == 'hygon':")
        with code.indent():
            code.writeline("if args['HYGON_LARGE_MUL']:")
            with code.indent():
    ...
    with code.indent():
        if rank > 0:
            ...
            code.writeline("FAST_2D_LAST: tl.constexpr,")
            code.writeline("HYGON_LARGE_MUL: tl.constexpr,")
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            if rank == 2:
                code.writeline("if FAST_2D_LAST:")
                with code.indent():
                    code.writeline("idx_offsets = offsets")
                    code.writeline("src_offsets = offsets")
                    code.writeline("if INT32_OFFSET:")
                    with code.indent():
                        code.writeline("shape_1 = shape_1.to(tl.int32)")
                        code.writeline("inp_stride_0 = inp_stride_0.to(tl.int32)")
                    code.writeline("row = offsets // shape_1")
                    code.writeline("inp_offsets = row * inp_stride_0")
                code.writeline("else:")
                with code.indent():
                    _generate_generic_offsets(code, rank)
            else:
                _generate_generic_offsets(code, rank)
...
def generate_destination_passing_wrapper(
    ...
    with code.indent():
        ...
        if rank == 2:
            code.writeline("FAST_2D_LAST = (")
            with code.indent():
                code.writeline("inp_strides[1] == 0")
                code.writeline("and stride_dim == 1")
                code.writeline("and inp_strides[0] == inp_size_dim")
                code.writeline("and index_strides[1] == 1")
                code.writeline("and index_strides[0] == index_shapes[1]")
                code.writeline("and src_strides[1] == 1")
                code.writeline("and src_strides[0] == index_shapes[1]")
            code.writeline(")")
        else:
            code.writeline("FAST_2D_LAST = False")
        code.writeline("HYGON_LARGE_MUL = (")
        with code.indent():
            code.writeline("flag_gems.vendor_name == 'hygon'")
            code.writeline("and IS_MUL")
            code.writeline("and N >= 16777216")
        code.writeline(")")
