...
def linalg_solve_triangular_impl(
    ...
    if HAS_TLE:
        ...
        # fp64 bs64 is impossible: the update dot stages a_sub (128x64 f64
        # = 64 KB) in smem, alone at the 64 KB CTA limit on Hygon/Metax --
        # n=512 fp64 compiled to 81920 B > 65536 B (OutOfResources). fp64
        # stays at bs32 (a_sub 32 KB + x_panel 4 KB fits); ks16 is kept at
        # n>=512 (only widens x_panel's K dim, cheap in smem).
        BLOCK_SIZE = 64 if (n >= 512 and not is_fp64) else 32
