...
def indice_freq_kernel(
    ...
    current_freq = tl.load(indices_freq + index_element, mask=mask, other=0)
    tl.store(indices_freq + index_element, current_freq + 1, mask=mask)
...
def embedding_backward_kernel(
    ...
    if not HAS_PADDING_IDX:
        ...
        current_grad = tl.load(grad_in + cols, mask, other=0.0).to(tl.float32)
        new_grad = current_grad + embedding_grad
        tl.store(grad_in + cols, new_grad, mask=mask)
    else:
