...
@libentry()
@triton.jit(do_not_specialize=["eps"])
def fused_add_rms_norm_c_split_kernel(
    input_ptr,  # pointer to the input
    residual_ptr,  # pointer to the residual
    w_ptr,  # pointer to the weights
    in_stride_r,  # how much to increase the pointer when moving by 1 row
    ...
    for off in range(0, N, BLOCK_SIZE):
        ...
        mask = cols < N
        x = tl.load(input_ptr + cols, mask, other=0.0).to(cdtype)
        r = tl.load(residual_ptr + cols, mask, other=0.0).to(cdtype)
        x += r
        _var += x * x / N
    ...
    for off in range(0, N, BLOCK_SIZE):
        ...
        r = tl.load(residual_ptr + cols, mask, other=0.0).to(cdtype)
        x += r
        tl.store(residual_ptr + cols, x, mask=mask)
...
def fused_add_rms_norm(x, residual, normalized_shape, weight, eps=1e-5):
    ...
    with torch_device_fn.device(x.device):
        if BLOCK_SIZE <= 1024:
            ...
            BLOCK_SIZE = 1024
            fused_add_rms_norm_c_split_kernel[M,](
                x,
                residual,
                weight,
                N,
